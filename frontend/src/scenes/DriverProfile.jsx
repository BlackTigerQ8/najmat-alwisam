...
const DriverProfile = ({ driverId }) => {
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      await dispatch(
        updateDriver({ driverId: driverInfo._id, formData })
      );
    } catch (error) {
  ...
  const handleViewFile = (values) => {
    ...
    const fileUrl = values.uploadedFile ? URL.createObjectURL(values.uploadedFile) : `${process.env.REACT_APP_API_URL}/${driverInfo.file}`;
